<?php
	...
	function find_all_sups($public=true) {
		global $connection;
	...
	function find_all_exceptions($public=true) {
		...
		$query  = "SELECT * ";
		$query .= "FROM exceptions ";
		$query .= "ORDER BY id";
		$exception_set = mysqli_query($connection, $query);
		confirm_query($exception_set);
		return $exception_set;
	}
	
	function find_exceptions_for_sup($sup_id, $public=true) {
		global $connection;
		...
		$query  = "SELECT * ";
		$query .= "FROM exceptions ";
		$query .= "WHERE sup_id = {$safe_sup_id} ";
		if ($public) {
			$query .= "AND visible = 1 ";
		}
		$query .= "ORDER BY position ASC";
	...
	function find_exception_by_id($exception_id, $public=true) {
		global $connection;
		
		$safe_exception_id = mysqli_real_escape_string($connection, $exception_id);
		
		$query  = "SELECT * ";
		$query .= "FROM exceptions ";
		$query .= "WHERE id = {$safe_exception_id} ";
		$query .= "LIMIT 1";
		$exception_set = mysqli_query($connection, $query);
		confirm_query($exception_set);
		if($exception = mysqli_fetch_assoc($exception_set)) {
			return $exception;
		} else {
			return null;
		}
